/**
  ...
  $Revision: 42948 18db16d6a91dc1b324e18367ed37053e7bcec12d $
  <- Information above is from upstream revision (Autodesk).

  $Date: 2021-09-24 00:00:00 $

  
  Modified from generic Fanuc post to support MotusCNC.
  
  Change history:
  
  Rev. date         who     description
  ----------------  ------  -------------------------------------
  2018-09-28        SJH     Initial
  2018-11-26        SJH     Add G69 at end of rotated sections
  2018-12-27        SJH     Work plane fixes
  2019-04-11        SJH     Write G49 to cancel TCPC mode at end.
  2020-09-28        SJH     Job posted as '3d' output 5-axis header
                            in case running on machine with trunnion.
                            Also add properties to set "home" AC position.
  2020-10-09        SJH     Merged in upstream changes.
  2020-10-16        SJH     Optimize retract between similar work planes.
  2020-10-23        SJH     Fix initial XY move in multi-axis sections.
  2020-10-28        SJH     Reset motion mode at end of canned cycles.
  2021-02-08        SJH     Removed "Cannot cancel length compensation if the machine is not fully retracted." check
  2021-09-24        SJH     Emit g69 after retracting for new work plane.
  2021-10-11        SJH     Emit M105 P91 at start of each section instead of job.  Depends on whether simul. 5 axis.
                            Also emit G91.4 to force wrap-around C axis.
  2021-12-16        SJH     Emit Z when g43.4 to avoid soft limit errors and excessive moves, also pre-position A,C.
  2022-02-14        SJH     Change 2021-09-24 fixed to properly cancel workplane.
  2022-03-08        SJH     Fix coolant codes to allow mist, remove unimplemented.
  2022-05-05        SJH     Ensure XY move before Z move in g43.x block.
*/

description = "MotusCNC";
vendor = "MDA Precision";
vendorUrl = "http://www.mdaprecision.com";
legal = "Copyright (C) 2012-2020 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 40783;
debugMode = true; 
setWriteInvocations = true;
longDescription = "Generic post for MotusCNC.";
...
properties = {
  ...
  smartRetract: true, // specifies that G0.1 'smart retract' should be used instead of G28 (between sections, when no tool change etc.)
  useG54x4: true, // Fanuc 30i supports G54.4 for Workpiece Error Compensation 
  ...
  verboseComments: true, // Write additional comments to output for debugging the post processor script
  zeroAC5Axis: true, // Write G0A_C_ at beginning and end of multi axis program (use next to get coords, not always 0)
  homeA: 0.,	// Initial and final A axis position
  homeC: 0.	// ...and C.
};
...
propertyDefinitions = {
  ...
  useG54x4: {title:"Use G54.4", description:"MotusCNC supports G54.4 for workpiece error compensation.", type:"boolean"},
...
*/
  ...
  smartRetract: {title: "G0.1 Smart retracts", description: "Use G0.1 instead of G28 when appropriate", type: "boolean"},
  verboseComments: {title: "Verbose comments", description: "Write additional comments to output for debugging the post processor script.", type: "boolean"},
  zeroAC5Axis: {title: "Set rotary", description: "Retract Z then move rotary axes to home position at start and end of program.", type: "boolean"},
  homeA: {title: "Home A position", description: "A axis position at start and end of program.", type: "float"},
  homeC: {title: "Home C position", description: "C axis position at start and end of program.", type: "float"}
};
...
// *OF formats for when we want to force the output e.g. in comments
var xOF = createFormat({prefix:" X", decimals:(unit == MM ? 3 : 4), forceDecimal:true});
var yOF = createFormat({prefix:" Y", decimals:(unit == MM ? 3 : 4), forceDecimal:true});
var zOF = createFormat({prefix:" Z", decimals:(unit == MM ? 3 : 4), forceDecimal:true});
var aOF = createFormat({prefix:" A", decimals:3, forceDecimal:true, scale:DEG});
var bOF = createFormat({prefix:" B", decimals:3, forceDecimal:true, scale:DEG});
var cOF = createFormat({prefix:" C", decimals:3, forceDecimal:true, scale:DEG});
...
var g68_2_activeSection = false;
var angularProbingMode;
var subprograms = [];
var currentPattern = -1;
var firstPattern = false;
var currentSubprogram;
var lastSubprogram = 2000;    //TODO: make this a property.  Starting subroutine number (minus 1).
...
function verbosePostProcessorDump() {
  // Dump interesting info about this job (from onOpen) as comments to the output.
  // For MotusCNC, generate backtick comments which enable use of smart retract etc.
  writeln("");
  writeComment("`job " + (is3D() ? "3d" : "multiaxis"))
  var bb;
  if (isWorkpieceDefined()) {
    bb = getWorkpiece();
    writeComment("`workpiece lower: "+xOF.format(bb.lower.x)+yOF.format(bb.lower.y)+zOF.format(bb.lower.z))
    writeComment("`workpiece upper: "+xOF.format(bb.upper.x)+yOF.format(bb.upper.y)+zOF.format(bb.upper.z))
  }
  bb = getFixture();
  writeComment("`fixture lower: "+xOF.format(bb.lower.x)+yOF.format(bb.lower.y)+zOF.format(bb.lower.z))
  writeComment("`fixture upper: "+xOF.format(bb.upper.x)+yOF.format(bb.upper.y)+zOF.format(bb.upper.z))
...
function onOpen() {
  ...
  if (true) { // note: setup your machine here
    var aAxis = createAxis({coordinate:0, table:true, axis:[-1, 0, 0], range:[-101, 120], preference:1});
    var cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], range:[-180, 180], preference:0, cyclic:true});
  ...
  writeComment("Program: " + String(programId));

  writeComment(localize(longDescription) + "  Post rev. date: "+getHeaderDate());

  verbosePostProcessorDump();
  ...
  writeBlock(gAbsIncModal.format(90), 
              gFeedModeModal.format(properties.useG95 ? 95 : 94), 
              gPlaneModal.format(17), 
              gFormat.format(49), 
              gFormat.format(69),   // 2018-12-27 - add g69 at start to ensure default work plane
              gFormat.format(40), 
              gFormat.format(80), 
              gFormat.format(91.1), // Incremental arc I,J coords
              gFormat.format(91.4) // 2021-10-11 C axis wrap-around
              );
  ...
  //sjh 20200928 - do this even if only '3d' job, this post processor assumes running on trunnion.
  // Orient rotary to zero and/or unwind rotary beta at start
  writeBlock("M119", "P30")
  if (properties.zeroAC5Axis) {
    doSmartRetract = false;
    writeComment("Retract for initial rotary home")
    writeRetract(Z);
    writeBlock(gMotionModal.format(0), aOutput.format(properties.homeA*Math.PI/180.), cOutput.format(properties.homeC*Math.PI/180.));
  }
...
var tcpc = 0;           // record whether g49/g43 (0), g43.4 (1) or g43.5 (2) in effect.
var retracted = false; // specifies that the tool has been retracted to the safe plane

/** Disables length compensation if currently active or if forced. */
function disableLengthCompensation(force) {
  if (lengthCompensationActive || force) {
    // validate(retracted, "Cannot cancel length compensation if the machine is not fully retracted.");  SJH 2021-02-08
...
function defineWorkPlane(_section, _setWorkPlane) {
  ...
  if (forceMultiAxisIndexing || !is3D() || machineConfiguration.isMultiAxisConfiguration()) { // use 5-axis indexing for multi-axis mode
    ...
    // 2022-02-14 always do this in 5-axis
    if (_setWorkPlane)
      forceWorkPlane();
    ...
    if (_section.isMultiAxis()) {
      ...
      if (machineConfiguration.isMultiAxisConfiguration()) {
        ...
        if (_setWorkPlane) {
          if (!retracted) {
            writeComment("Retract for work plane in multi-axis")
            writeRetract(Z);
          }
      ...
      // Now uniformly support g68.2 workplane
      var euler = _section.workPlane.getEuler2(EULER_ZXZ_R);
      c = euler.x + Math.PI;
      if (c > Math.PI)
        c = c - Math.PI*2;
      writeComment("angles: " + "A" + abcFormat.format(euler.y) + " C" + abcFormat.format(c))
...
function setWorkPlane(abc) {
  ...
  onCommand(COMMAND_UNLOCK_MULTI_AXIS);
  if (!retracted) {
    writeComment("Retract for set work plane")
    writeRetract(Z);
  }
  ...
  g68_2_activeSection = true;
  forceXYZ();   // 2018-12-27 - g68.2 etc. can upset XYZ so force output
...
function vbcomment(s) {
  if (properties.verboseComments) {
    writeComment(s);
  }
}

function verboseSectionDump(s) {
  // Dump interesting info about this section as comments to the output.
  // Always generate backtick comments, but verbose flag enables some others which are not significant to interpreter.
  var vb = properties.verboseComments;
  ...
  writeComment("`isMultiAxis: "+s.isMultiAxis());
  if (vb) {
    writeComment("isOptional: "+s.isOptional());
    writeComment("isPatterned: "+s.isPatterned());
    writeComment("isOriented X:"+s.isXOriented()+" Y:"+s.isYOriented()+" Z:"+s.isZOriented());
    var opt = s.isOptimizedForMachine();
    writeComment("isOptimizedForMachine: "+opt);
    writeComment("OptimizedTCPMode: "+s.getOptimizedTCPMode());
  }
  var bb = s.getBoundingBox();
  writeComment("`sectbb lower: "+xOF.format(bb.lower.x)+yOF.format(bb.lower.y)+zOF.format(bb.lower.z))
  writeComment("`sectbb upper: "+xOF.format(bb.upper.x)+yOF.format(bb.upper.y)+zOF.format(bb.upper.z))
  var fp, fa, ip, ia, faa, iaa;
  if (s.hasWellDefinedPosition()) {
    ip = s.getInitialPosition();
    ia = s.getInitialToolAxis();
    fp = s.getFinalPosition();
    fa = s.getFinalToolAxis();
    writeComment("`IPOS: "+xOF.format(ip.x)+yOF.format(ip.y)+zOF.format(ip.z))
    writeComment("`IDIR: "+xOF.format(ia.x)+yOF.format(ia.y)+zOF.format(ia.z))
    writeComment("`FPOS: "+xOF.format(fp.x)+yOF.format(fp.y)+zOF.format(fp.z))
    writeComment("`FDIR: "+xOF.format(fa.x)+yOF.format(fa.y)+zOF.format(fa.z))
    if (opt) {
      iaa = s.getInitialToolAxisABC();
      faa = s.getFinalToolAxisABC();
      writeComment("`IABC: "+aOF.format(iaa.x)+bOF.format(iaa.y)+cOF.format(iaa.z))
      writeComment("`FABC: "+aOF.format(faa.x)+bOF.format(faa.y)+cOF.format(faa.z))
    }
  }
  writeln("");
}
...
function onSection() {
  ...
  verboseSectionDump(currentSection);
  
  // Flag for writeRetract() to allow it to use G30.
  doSmartRetract = !insertToolCall;
  
  // Flag to use G0.1 instead of full retract.  Used if relatively small orientation change.
  do_G0_1 = !isFirstSection() && !insertToolCall && !newWorkOffset && currentSection.isOptimizedForMachine() &&
            Vector.diff(getPreviousSection().getFinalToolAxis(), currentSection.getInitialToolAxis()).length < 0.2;
  
  if (currentSection.isMultiAxis()) {
    // Do not attach workpiece coordinate system to rotary table in simul. 5 axis modes.
    writeBlock("M105", "P91", "Q0")
  }
  else {
    // Attach workpiece coordinate system to rotary table.  This allows g43 to
    // work with g68.2+g53.1 workplane setting for indexed work.  This mode is 
    // automatically reset to user-specified default on rewind.
    writeBlock("M105", "P91", "Q1")
  }
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane || forceSmoothing) {
    ...
    writeComment("Retract for"+conditional(insertToolCall," tool-change")+conditional(newWorkOffset," WCS")+conditional(newWorkPlane," work-plane"))
    ...
    cancelWorkPlane();  // 2021-09-24, 2022-02-14
  ...
  if (insertToolCall) {
    ...
    if (properties.verboseComments) {
      if (is3D()) {
        ...
        writeComment(localize("ZMIN [global]") + "=" + zRange.getMinimum());
      }
    }
  ...
  // Conventional Fixture Offsets (a.k.a WCS)
  ...
  if (workOffset > 0) {
    if (workOffset > 9) {           // We support g59.1,2,3 for offsets 7,8,9.
      error(localize("Work offset out of range."));
      return;
    } else if (workOffset > 6) {
      if (workOffset != currentWorkOffset) {
        writeBlock(gFormat.format(59 + (workOffset-6)*0.1)); // G59.1,.2,.3 
  ...
  if (insertToolCall || !lengthCompensationActive || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
        ...
        // cancel compensation prior to enabling it, helps avoid soft limit errors when TCPC tries to follow large rotation.
        disableLengthCompensation(false);

        abc = defineWorkPlane(currentSection, newWorkPlane && !currentSection.isMultiAxis());
        ...
        if (!machineConfiguration.isHeadConfiguration()) {
          ...
          forceXYZ();   // 2018-12-27 - g43 etc. can upset XYZ so force output

          if (offsetCode == 43.4 || offsetCode == 43.5) {
              iaa = currentSection.getInitialToolAxisABC();
              writeBlock(
                gMotionModal.format(0),
                aOutput.format(iaa.x),
                cOutput.format(iaa.z)
              );
              writeBlock(
                gMotionModal.format(0),
                ...
                //zOutput.format(initialPosition.z),    // 2022-05-05 do Z move next block, below.
          ...
          // 2022-05-05 - foll. block applies to both cases above (was in just the 'else' case)
          //TODO: it would possibly be better for the interpreter to move like G0.1 when actioning a g43.x position,
          // but for now just move Z on a separate block.
          // 2018-12-27 - Z move after XY, to avoid potential ramp-down after tool change.
          writeBlock(gMotionModal.format(0), zOutput.format(initialPosition.z));
          lengthCompensationActive = true;
        } else {
          error(localize("Head rotation machine configuration not supported."));
          return;
          /*
          writeBlock(
            gAbsIncModal.format(90),
        ...
        tcpc = offsetCode == 43.4 ? 1 : offsetCode == 43.5 ? 2 : 0; 
        zIsOutput = true;

        gMotionModal.reset();

  } else {
        /* 2018-12-27 swapped order */
        abc = defineWorkPlane(currentSection, newWorkPlane);
        writeBlock(
          gAbsIncModal.format(90),
          gMotionModal.format(0),
          xOutput.format(initialPosition.x),
  ...
  if (do_G0_1) {
    ...
    iaa = currentSection.getInitialToolAxisABC();
    writeComment("G0.1 "+xOutput.format(initialPosition.x)+yOutput.format(initialPosition.y)+zOutput.format(initialPosition.z)+
                 aOutput.format(iaa.x)+cOutput.format(iaa.z));
  }
  ...
  //retracted = false;  // no, this is falsified when Z next moves.
}
...
function onCyclePoint(x, y, z) {
  ...
  // canned drilling cycles and patterens not cooperating this should foce all 
  // if (true || isProbeOperation()){
...
function onCycleEnd() {
  ...
  if (cycleSubprogramIsActive) {
    ...
    // 2020-10-28 - make it output G0 next move, and don't use G80.
  if (!cycleExpanded) {
      writeBlock(conditional(!properties.useG95, gFeedModeModal.format(94)));
    zOutput.reset();
      gMotionModal.reset();
    }
  }
...
function onSectionEnd() {
  ...
  //writeBlock(gPlaneModal.format(17), conditional(g68_2_activeSection, gFormat.format(69)));
  //g68_2_activeSection = false;
  //currentWorkPlaneABC = undefined;  // 2018-12-27 - force output of next w.p. setting
...
function onClose() {
  ...
  doSmartRetract = false;
  writeComment("Retract for close")
  writeRetract(Z); // retract

  disableLengthCompensation(false); // 2019-04-11 SJH - write G49 to cancel TCPC mode at end.
  
  // Be kind: unwind
  //sjh 20200928 - do this even if only 3d job, may be running on trunnion.
  //if (!is3D()) {
    writeBlock("M119", "P30")
    if (properties.zeroAC5Axis) {
      if (tcpc == 2) {
        // If type 2 TCPC mode, cancel so we can write g0a0c0.
        disableLengthCompensation(false);
      }
      writeBlock(gMotionModal.format(0), aOutput.format(properties.homeA*Math.PI/180.), cOutput.format(properties.homeC*Math.PI/180.));
    }
  ...
  //sjh 20200928 - just output a g69, not a g53.1 since don't want to move AC.
